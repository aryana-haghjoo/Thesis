...
Ultimately, this thesis focuses on a specific parameter estimation method. We adopt the Markov Chain Monte Carlo (MCMC) method combined with the Levenberg Marquardt (LM) algorithm to estimate the best-fit physical parameters (e.g., clumping factor, star formation efficiency) of the theoretical 21cm curves. We use the Accelerated Reionization Era Simulations (ARES) to generate models for the global 21-cm signal. Our method is flexible to the choice of parameters from ARES.\par
...
When the CMB photons redshift through expansion of universe, there comes a period when they reach wavelengths close to that of hydrogen intrinsic bands. During this period, hydrogen molecules are able to absorb CMB photons (which later, in chapter \ref{chap:global21cm}, will be recalled as coupling of spin temperature $T_s$ with kinetic temperature of the gas $T_k$). This mechanism happens roughly during the dark ages/epoch of reionization (EoR) and will leaves the brightness temperature of 21cm line affected. This change can be detected through precise observation using modern radio interferometers \footnote{As a result of redshift, the 21cm signal reaches frequencies of order 30-200MHz, making it a reachable target for our radio interferometers\cite{low_frequency}}. These observations are capable of giving us valuable information about the distribution of neutral hydrogen and evolution of cosmic structure over time\cite{low_frequency}.\par
