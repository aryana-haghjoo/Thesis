...
\item $40 \lesssim z \lesssim 80$: \hl{As the density keeps dropping through the expansion, the influence of collisional couplings becomes less significant, causing the spin temperature to again couple with the} \gls{cmb} temperature ($T_S \rightarrow T_\gamma$). Thus, the $21cm$ differential brightness signal goes back to zero again\cite{map_universe}. The first blue regions in the upper panel of figure \ref{fig:global_signal_pritchard_loeb} and the corresponding absorption trough in the lower panel depict the above-mentioned process \cite{map_universe, 21century}.\par
...
\begin{figure}[h!]
    ...
    \caption[Evolution of the global $21cm$ signal generated using \gls{21cmfast}]{Evolution of the  global $21cm$ signal generated using \gls{21cmfast}. Solid, dotted, and dashed curves represent the spin temperature, CMB temperature ($T_\gamma$ or equivalently $T_{CMB}$), and the kinetic temperature of the gas, respectively. \hl{Note that this figure displays the brightness temperature of $21cm$ signal} while figures \ref{fig:global_signal_pritchard_loeb} and \ref{fig:ares_Curve} show differential brightness temperature. Figure from \cite{21cmfast_python}.}
...
%-----------------------------------------------------------------------------
\section{Discussion}
Constraining the period between the dark ages and reionization with observations of the global 21 cm signal has been the focus of many studies. Therefore, they have conducted many different parameter estimation methods.
In \cite{21cmpower_global_comnbine}, authors suggest that we can benefit from analyzing the data from a combined 21 cm power spectrum and global signal measurements. This parameterization reduces the number of parameters needed to describe the global signal, increasing the likelihood of a high-significance measurement.
Adding 21 cm power spectrum information to existing constraints results in reduced errors on many cosmological parameters (in addition to producing the first measurements of the astrophysical parameters governing reionization). The further addition of global signal information, however, does little to improve parameter errors.\par

In \cite{pe_galaxy_formation}, authors claim the first attempt to forecast constraints on astrophysical parameters of interest from mock observations of the global 21-cm signal. Constraints on the turning points constrain the model parameters well. Such measurements would simultaneously constrain the ionization and thermal state of the IGM. The used parameters: minimum virial temperature of star-forming $T_{min}$, \gls{lya} efficiency $\xi_{LW}$, X-ray efficiency $\xi_{X}$, Ionizing efficiency $\xi_{\text{ion}}$\par

\cite{subtle_lya} Contrast to EDGES: new sources of heating: subtle effects of \gls{lya} radiation \gls{lya} heating, CMB heating (mediated by \gls{lya} photons), and multiple scattering of \gls{lya} photons. 165 mK at redshifts $z \approx 15-19$. This is in contrast with the predictions for a pure adiabatically cooling Universe, for which the deepest possible absorption is a monotonically decreasing function of cosmic time and is $-178 mK$ at z = 19 and $-216 mK$ at z = 15, dropping to even lower values at lower redshifts. 6 astrophysical parameters. although created by subtle physical processes, the newly included effects of the \gls{lya} background on the 21cm signal are important. The landscape of possible 21-cm signals is strongly modified when these effects are included,\par

\cite{chart_param_space} Even with our wide selection of models, we still find clear correlations between the key features of the global 21cm signal and underlying astrophysical properties of the high redshift Universe, namely the \gls{lya} intensity, the X-ray heating rate, and the production rate of ionizing photons. These correlations can be used to directly link future measurements of the global 21cm signal to astrophysical quantities in a mostly model-independent way. We identify additional correlations that can be used as consistency checks. 4 astrophysical parameters\par

\cite{pe_mcmc_1}five cosmological and seven related to the stellar populations. MCMC)-based parameter estimation package, CosmoReionMC, when we include the EDGES data in the analysis, we find that an early population of metal-free stars with efficient radio emission is necessary to match the absorption amplitude. The CosmoReionMC, require a radio-efficient short-lived population of metal-free (PopIII) stars which give rise to the absorption trough. The inclusion of the PopIII stars allow reionization histories that start earlier than what is found in the CMB+Quasar analysis. Our model can match the position and depth of the EDGES absorption trough, however, matching the exact shape would require more complex evolution of the PopIII star formation than what is assumed in this paper.\par

\cite{pe_nn_1} we have used an artificial neural network (ANN) to extract the astrophysical parameters from these simulated data sets, We find that the reconstructed signal closely mimics the amplitude of the reported detection. The recovered parameters can be used to infer the physical state of the gas at high redshifts. We can see that by training our ANN with models, which include EDGES-like large absorption troughs apart from the traditional models, we get the predicted parameters that are used to reconstruct the signal. We can see that the magnitude of the signal is comparable to the EDGES detection, but the flattened nature of the EDGES signal cannot be reconstructed by the signal reconstructed by our models. parameters: $f_*, f_{esc}, f_X, f_{XH}, f_R, N_\alpha$, $f_R$ excess radio background.\par
