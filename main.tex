...
\begin{titlepage}
   ...
   the Global $\mathrm{21cm}$ Signal}
			...
			\includegraphics[width=0.6\textwidth]{figures/McGill_logo.png}
...
\emph{To all lifelong learners and doers: you are changing the world every day!}
 %---------------------------------------------------------------------------------------------------------------
 \chapter*{Contribution of Authors}\markboth{Contribution of Authors}{}
...
%\footnote{It is worth emphasizing that in this thesis, we will only focus on the global $\mathrm{21cm}$ signal as the subject of study.}
%-----------------------------------------------------------
...
\begin{figure}[h!]
\centering
\includegraphics[scale =0.4]{figures/spinflip.jpg}
...
As previously mentioned, a hydrogen atom in the early universe is able to interact with \gls{cmb} background through photon exchange. However, with the onset of star formation, the \gls{cmb} will no longer be the only source of photons for the hydrogen atom to interact with. The newly formed objects (massive stars) are capable of generating \gls{uv} photons and reheating the \gls{igm}. This excess heat is responsible for the increased collisions, triggering the ionization and de-excitation of \gls{igm} gas. This process generates a new source of photons that are injected into the \gls{lya} frequency band by resonant scattering rather than being redshifted from outside of the line. The existence of this second source of the photons increases the probability of the hydrogen-background interaction, further assisting the spin temperature $T_S$ in coupling to $T_K$ \cite{21century, low_frequency}.\par
...
\item $z_\alpha \lesssim z \lesssim z_*$: As large halos begin to collapse and form first stars and galaxies, they generate X-ray emission. These photons are capable of heating their surrounding gas and elevating 
 the probability of collisions. On the other hand, two mechanisms create \gls{lya} photons: 1) Continuous redshifting of background radiation to the \gls{lya} wavelength (continuum photons), and 2) Collisional ionization and deexcitation of \gls{igm} gas (injected photons). As a result, a \gls{lya} background is formed. Due to the high cross-section of \gls{lya} photons and hydrogen atoms, the spin temperature tends to couple to $T_k$. Therefore, we will again observe the global $\mathrm{21cm}$ signal in absorption (the second trough in the lower panel of Figure \ref{fig:global_signal_pritchard_loeb}) \cite{map_universe, 21century}.\par
...
On top of these simulators, some studies have proposed the use of emulators \cite{emulate_21cm, emulate_21cm_2, emulate_21cm_3}. These statistical models are trained using the output data of simulators. They emulate the behavior of simulators of global $\mathrm{21cm}$ signal by learning the complex relationships between input parameters and the corresponding output. Emulators create a fast and approximate surrogate model of the simulator, which can be used to predict the outcome for new input parameter values. While emulators allow rapid explorations of parameter space, it should be noted that they are inherently approximations of the underlying physics. Thus, they may not capture all the fine details of the system's behavior.\par
In this section, we introduce two powerful tools capable of simulating the theoretical $\mathrm{21cm}$ curves over a wide range of astrophysical and cosmological parameters. The first one is \gls{21cmfast} \cite{21cmfast_c, 21cmfast_python}, which is a tool to simulate the spatial $\mathrm{21cm}$ signal. It is also capable of generating the global curve by averaging over all the signals in different directions. The other one is \gls{ares}, which calculates the global curve directly by solving the cosmological \gls{rte} (equation \ref{eq:RT}) \cite{ares2014jordan}. The main difference between these two comes from the fact that they use different parameterizations of the $\mathrm{21cm}$ theory as the basis of their simulations. For global $\mathrm{21cm}$ applications, the use of \gls{ares} is preferred over \gls{21cmfast} based on its direct approach for generating the global $\mathrm{21cm}$ curve. \par
...
\begin{figure}[h!]
    \centering
    \includegraphics{figures/21cm_dm.jpg}
...
\begin{figure}[h!]
    \centering
    \includegraphics[scale = 0.8]{figures/21cm_cosmic_string.jpg}
...
\begin{figure}[h!]
    \centering
    \includegraphics[scale= 0.75]{figures/edges_data.png}
...
\begin{figure}[h!]
\centering
\includegraphics[scale =0.7]{figures/LM_flow.png}
...
\begin{figure}[h!]
\centering
\includegraphics[scale =0.9]{figures/MCMC_flow.png}
...
\begin{figure}[h!]
\centering
\includegraphics[scale =0.9]{figures/mcmc_uncoverged.jpg}
...
\begin{figure}[h!]
\centering
\includegraphics[scale =0.9]{figures/mcmc_converged.jpg}
...
\begin{figure}[h!]
\centering
\includegraphics[scale =0.9]{figures/combined_flow.png}
...
\begin{figure}[h!]
\centering
\includegraphics[scale =0.6]{figures/corner_plots.png}
...
\begin{figure}[h!]
\centering
\includegraphics[scale =0.8]{figures/csq_hist.png}
...
\begin{figure}[h!]
\centering
\includegraphics[scale =0.5]{figures/csq_vs_params.png}
...
\begin{enumerate}
    ...
    \item \bm{$N_{lw}$}: \emph{Number of photons emitted in the Lyman-Werner band ($\mathrm{11.5eV<E<13.6eV}$) per baryon of star formation}. This parameter is referred to as \emph{pop\_rad\_yield\_0\_} in ARES documentation \cite{ares_documentation, lw_background}.
    
    \item \bm{$N_{ion}$}: \emph{Mean number of ionizing photons ($E>\mathrm{13.6eV}$) produced per baryon of star formation}. This parameter is referred to as \emph{pop\_rad\_yield\_2\_} in ARES documentation \cite{ares_documentation, 21century}.
...
\begin{figure}[h!]
\centering
\includegraphics[scale =0.5]{figures/sensivity.png}
...
\begin{figure}[h!]
\centering
\includegraphics[scale =0.7]{figures/fit_curve_mock.png}
...
\begin{figure}[h!]
\centering
\includegraphics[scale =0.5]{figures/chain_known_curve.png}
...
\begin{figure}[h!]
\centering
\includegraphics[scale =0.5]{figures/power_spectrum_known_curve.png}
...
\begin{figure}[h!]
\centering
\includegraphics[scale =0.7]{figures/csq_hist_known_curve.png}
...
\begin{figure}[h!]
\centering
\includegraphics[scale =0.5]{figures/csq_vs_params_known_curve.png}
...
\begin{figure}[h!]
\centering
\includegraphics[scale =0.5]{figures/csq_vs_params_zoomed_known_curve.png}
...
\begin{figure}[h!]
\centering
\includegraphics[scale =0.6]{figures/corner_plots_known_curve.png}
...
\begin{figure}[h!]
\centering
\includegraphics[scale =0.7]{figures/fit_curve_edges.png}
...
\begin{figure}[h!]
\centering
\includegraphics[scale =0.5]{figures/chain_edges.png}
...
\begin{figure}[h!]
\centering
\includegraphics[scale =0.5]{figures/power_spectrum_edges.png}
...
\begin{figure}[h!]
\centering
\includegraphics[scale =0.7]{figures/csq_hist_edges.png}
...
\begin{figure}[h!]
\centering
\includegraphics[scale =0.5]{figures/csq_vs_params_edges.png}
...
\begin{figure}[h!]
\centering
\includegraphics[scale =0.5]{figures/csq_vs_params_zoomed_edges.png}
...
\begin{figure}[h!]
\centering
\includegraphics[scale =0.5]{figures/corner_plots_edges.png}
...
\begin{comment}
The unique feature which distinguishes our study from these previous similar analyses is the employment of a simulation method solely developed for global $\mathrm{21cm}$ applications. Therefore, we claim that our approach to estimating the astrophysical parameters of this signal is one of the most computationally-efficient methods.\par
\end{comment}
