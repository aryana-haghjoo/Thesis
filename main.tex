...
Previous studies have examined various computational algorithms to accomplish this task. The efforts include \gls{mcmc} \cite{pe_mcmc_1, pe_mcmc_2}, neural network methods \cite{pe_nn_1}, and even Fisher Matrix approaches \cite{pe_fisher_1, pe_fisher_2}. Sampling algorithms such as \gls{mcmc} are capable of exploring the entire likelihood surface in situations where it is feasible. On the other hand, neural networks are typically employed for problems where comprehensive mapping of the entire likelihood surface becomes excessively complex. This distinction positions \gls{mcmc} as advantageous for global 21cm applications.\par
...
The off-diagonal elements of a covariance matrix correspond to the covariance between each pair of parameters. Thus, by drawing samples from the covariance matrix, we are genuinely sampling from the multivariate normal distribution with the deviation values describing the uncertainties in the parameters\footnote{Figure \ref{fig:corner_plots}, which shows the corner plots for a four-variable Gaussian model, emphasizes the importance of sampling from a multivariate normal distribution and taking the correlations between the parameters into account.}.
...
\begin{figure}[h!]
\centering
\includegraphics[scale =0.8]{chi-sqaure test.jpg}
...
On this plot, the average of disparity is 3.97, and the standard deviation is 2.83, in acceptable agreement with our expectations. Therefore, the best-fit parameters are considered a "good fit".}
\label{fig:csq_test}
\end{figure}
...
Finally, we represent the results of parameter estimation for the half-amplitude \gls{edges} data\footnote{The reason behind choosing this value for the multiplication factor is discussed in section \ref{chap:results,sub:edges,subsub:m_factor}}. However, in agreement with our earlier expectation, even a multiplication factor of 0.5 is not a sufficient component to explain the behavior of \gls{edges} data with the predictions of the standard physics. \par
...
\end{enumerate}
Figure \ref{fig:sensivity} shows the influence of change in the values of the above-mentioned parameters on the global $21cm$ curve generated by \gls{ares}. It must be noted that these parameters are only physically meaningful in a limited region of phase space (A hypercube where the combination of parameters is physically meaningful). Going beyond this region will result in a failure in calculating the global $21cm$ curve. our developed script is designed to handle this issue and stop it from aborting the whole run \footnote{The Python code for the \gls{mcmc} is designed such that it will return an arbitrarily large chi-square for that combination of parameters beyond the permitted hypercube. Therefore, the probability of these points getting accepted into the chain will be automatically low.}, but the \gls{mcmc} might not be able to reach convergence if the algorithm faces a numerous samples beyond the allowed region.\par
%-----------------------------------------------------------------------------------
\section{Parameter Estimation of Mock Data}
\label{chap:results,sub:known_curve}
Prior to applying our developed algorithm to fit real-world data, we perform a validation test by using it to fit a known \gls{ares} curve. We create the \gls{ares} curve using a predetermined set of parameters ($\emph{pop\_rad\_yield\_0\_}=10^4, \emph{pop\_rad\_yield\_2\_}=10^3, f_{esc}=0.1, f_X= 0.1$), treating it as mock data. By comparing the parameter values obtained from the algorithm to the expected values within the range of the assumed error bars, we can confirm the correct functioning of the algorithm.\par
The results of this validation test are shown in table \ref{tab:mcmc_results_known_curve} and Figure{}. The output is an acceptable fit considering the overall shape and amplitude of the signal.
Figure \ref{fig:chain_known_curve} indicates the white-noise behavior of parameters throughout the chain, which is in agreement with our anticipation for a converged chain. Performing \emph{chi-Square consistency test} on our samples showed that the average disparity between the chi-square of drawn samples is 49.20, \hl{roughly an order of magnitude larger than the initial expectation ($\approx4$).}\par
We proceed by performing the \emph{chi-square parameter correlation test}. The results are depicted in Figure \ref{fig:csq_hist_known_curve}. In the hypothetical ideal case, we expect to see parabolic behavior. However, Figure \ref{fig:csq_vs_params_zoomed_known_curve}, which demonstrates the zoomed version of the \ref{fig:csq_hist_known_curve}, gives insights on parabolic behavior for lower values of chi-square. Figure \ref{fig:corner_plots_known_curve} also shows the correlation between different parameters of the model.\par
...
\hline
Error-bar of fit & $3.5417 \times 10^ {-2}$ & $3.8612 \times 10^ {0}$& $3.5782 \times 10^ {-4}$ & $3.3929 \times 10^ {-6}$ \\
\hline
...
This section demonstrates the results of applying our developed method to the \gls{edges} data, which is the only available observational data set of the global $21cm$ signal. Based on the fact that the large amplitude of this data is not completely explainable by standard physics and it is roughly more than two times higher than the most optimistic theoretical curves, we tend to use the \gls{edges} data with half the amplitude. This argument is supported by the analysis of the section \ref{chap:results,sub:edges,subsub:m_factor}, which calculates this multiplication factor using \gls{lm}.
Furthermore, as a matter of convenience in the first-step analysis, we refrain from employing the actual noise model used by the \gls{edges} group. Instead, we approximate the noise using the radiometer equation. The details of this approach is presented in section \ref{chap:results,sub:edges,subsub:error}.\par
\subsection{Choice of "multiplication factor"}
\label{chap:results,sub:edges,subsub:m_factor}
\subsection{error bar calculation}
\label{chap:results,sub:edges,subsub:error}
In order to obtain the error bars of the \gls{edges} data for the parameter estimation process, we can directly use the \gls{edges} noise model. Another option is to take advantage of an approximation. The radiometer equation expresses the sensitivity of a radio antenna as a function of three parameters \cite{sensitivity_1, sensitivity_2}.\par
\begin{equation}
    \frac{\delta T}{T_{sys}} = \frac{1}{\sqrt{Bt}}
\end{equation}
Where B is the bandwidth, t is the exposure time, and $T_{sys}$ is the system temperature. Assuming a bandwidth of $10^6Hz$, an exposure time of one day and temperature of $3000K$, we get: $\delta T = 10 ^{-2}K = 10mK$.

\subsection{results}
 The results of applying our method to \gls{edges} data are shown in table \ref{tab:mcmc_results_edges} and Figure{}. The output is not a perfect fit considering the overall shape and amplitude of the signal.
 Figure \ref{fig:chain_edges} indicates the white-noise behavior of parameters throughout the chain, which is in agreement with our anticipation for a converged chain. Performing \emph{chi-Square consistency test} on our samples showed that the average disparity between the chi-square of drawn samples is \hl{49.20, roughly an order of magnitude larger than the initial expectation ($\approx4$).}\par
 We proceed by performing the \emph{chi-square parameter correlation test}. The results are depicted in Figure \ref{fig:csq_hist_edges}. In the hypothetical ideal case, we expect to see parabolic behavior. Figure \ref{fig:corner_plots_known_edges} also shows the correlation between different parameters of the model.\par
...
say that the first step is to estimate the parameters of the standard curve, then we add the non-standard effects and do the parameter estimation again.
